...
var str='<li class="clearfix" id="c_'+res.commentid+'">';
			if(res.referid==0){
				if(res.count_approve>0){
					str += '<div class="'+(o.login==0?' need-login':'js-com-prove')+' com-prove '+(res.isapprove==0?'':' com-proved')+'" data-commentid="'+res.commentid+'" approved="'+res.approved+'">'+res.count_approve+'</div>';
				}else{
					str += '<div class="'+(o.login==0?' need-login':'js-com-prove')+' com-prove '+(res.isapprove==0?'':' com-proved')+'" data-commentid="'+res.commentid+'" approved="'+res.approved+'">赞同</div>';
				}
			}
			str += '<div class="com-img">'+
			...
			str +='<h4 class="title">'+
					...
					str += 
					(res.forbidden_comment == 1?'':('<a href="javascript:;" data-referid='+res.p_commentid+' data-replyuid="'+res.userid+'" data-replycommentid="'+res.commentid+'" class="fr reply'+(o.login==0?' need-login':'')+'">回复</a>'))+
